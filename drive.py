...
def crop_image(dataset) :
    return dataset[30:68,:,  :].copy()

def reformat(dataset):
    dataset = crop_image(dataset)
    dataset = change_to_gray_normalize(dataset)
    dataset = np.reshape(dataset, (-1, 38, 160, 1)).astype(np.float32) # 일단 1채널만
...
def telemetry(sid, data):
    ...
    image_array= cv2.resize(image_array, (160,80))  # resize
    transformed_image_array = reformat(image_array) # crop & grayscale
